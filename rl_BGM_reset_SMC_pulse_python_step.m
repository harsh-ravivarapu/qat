function [beta_vec, EI, CTX_workspace] = rl_BGM_reset_SMC_pulse_python_step(tmax,IT,pd,corstim,freq,pprofile,len,dt,stride,windowlen)
...
% Create DBS Current   (currently it is on STN)
...
if pattern==0
  ...
  [Idbs, ~]=creatdbs2(pattern,tmax,dt,PW,amplitude,pprofile);
end
...
[vgpi,vTH,SMC_pulse,CTX_workspace] = CTX_BG_TH_network(pd,corstim,tmax,dt,n,Idbs,Iappco); % [vgpi] = CTX_BG_TH_network(pd,corstim,tmax,dt,n,Idbs,Iappco);

% Calculate GPi pathological low-frequency oscillatory power
dt1=dt*10^-3; % not sure
params.Fs = 1/dt1; %Hz
params.fpass = [1 100];
params.tapers = [3 5];
params.trialave = 1;
...
% gpi_alpha_beta_area - GPi spectral power integrated in 13-35Hz band
% gpi_S - GPi spectral power
% gpi_f - GPi spectral frequencies


% calculate beta and EI
[beta_vec, EI] = sliding_window_2(vgpi,vTH,SMC_pulse,params,n,len,CTX_workspace.timespike,dt,stride,windowlen);
...
if isempty(pprofile)
    ...
    if impulse(end) > length(Idbs_step)-30
        impulse(end) = length(Idbs_step)-30;
    end
...
function [Idbs, impulse]=creatdbs(pattern,tmax,dt,PW,amplitude,pprofile)
...
if isempty(pprofile)
    ...
    if length(pprofile) ~= length(Idbs)
        impulse = impulse.*round(length(Idbs)/length(pprofile));
    end

    if impulse(end) > length(Idbs)-30
        impulse(end) = length(Idbs)-30;
    end

    for i = 1:length(impulse)
        Idbs(impulse(i):impulse(i)+pulse_w-1)=pulse;
    end
end
end

function [vgpi,vThal,SMC_pulse,CTX_workspace] = CTX_BG_TH_network(pd,corstim,tmax,dt,n,Idbs,Iappco) % function [vgpi] = CTX_BG_TH_network(pd,corstim,tmax,dt,n,Idbs,Iappco)
%Environment
    %time step
    t=0:dt:tmax;

    % SMC Pulse
    [SMC_pulse, timespike]= createSMC(tmax,dt,14,0.2);
...
function [beta_vec, error_index] = sliding_window_2(v,vth,SMC,params,n,len,timespike,dt,stride,windowlen)
%Used to calculate the beta and EI based on window size and stride determined from data 
    ...
    vpi_len = length(v(1,:));
    n_obs = floor((dt*(vpi_len-windowlen/dt-1))/stride);
    state_len = n_obs;
    ...
    while index <= length(beta_vec)
        ...
        ts_1 = timespike(timespike>t_hold(1));
        ts_1 = ts_1(ts_1<t_hold(end));
        error_index(index)=calculateEI(t_hold,vth_1,ts_1,t_hold(1),t_hold(end));
        j = j + stride;
        index = index + 1;
...
e=zeros(1,m);
b1=find(timespike>=tstart, 1 ); % DONT ignore first 200msec
...
function [Idbs, impulse] = createnormaldbs(pattern,tmax,dt,PW,amplitude,pprofile)

% creation of relevant arrays and variables 
t = 0:dt:tmax;
pulse_w = PW/dt; % pulse width

Idbs = zeros(1,length(t));
iD = amplitude;
pulse= iD*ones(1,pulse_w);

% original code: need to fix
% Loop through Idbs array and embed pulse array at random intervals while
% keeping the one second average frequence equal to the defined parameter
% above.
if isempty(pprofile)
